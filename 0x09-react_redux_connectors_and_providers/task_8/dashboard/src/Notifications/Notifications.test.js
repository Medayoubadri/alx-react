...
describe("Notifications", () => {
  ...
  describe("filter buttons", () => {
    it("clicking on the first button should call setNotificationFilter with URGENT", () => {
      const setNotificationFilterSpy = jest.fn();
      const wrapper = shallow(
        <Notifications
          displayDrawer={true}
          listNotifications={listNotifications}
          setNotificationFilter={setNotificationFilterSpy}
        />
      );

      // Find the first filter button (‼️ - urgent)
      const urgentButton = wrapper.find("button").at(1); // Skip the close button (index 0)
      urgentButton.simulate("click");

      expect(setNotificationFilterSpy).toHaveBeenCalledWith(
    ...
    it("clicking on the second button should call setNotificationFilter with DEFAULT", () => {
      ...
      // Find the second filter button (💠 - default)
      const defaultButton = wrapper.find("button").at(2); // Skip the close button (index 0) and urgent button (index 1)
